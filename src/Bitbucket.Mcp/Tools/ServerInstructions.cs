...
internal static class ServerInstructions
{
    ...
    internal const string Text = """
        ...
        - Diffs come in two steps. Call getPullRequestDiff with no paths to see which files changed, then call it again with paths=["..."] for the files you actually need — paths is what selects the diff, so mode never has to be set. A whole-pull-request diff fails outright on large pull requests, so never ask for one speculatively.
