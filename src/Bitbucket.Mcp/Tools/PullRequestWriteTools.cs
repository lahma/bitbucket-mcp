...
/// <summary>
/// The write half of the pull request surface: create, update, comment, review, merge, decline.
/// </summary>
/// <remarks>
/// <para>
/// The annotations are the load-bearing part of this file. <c>Destructive</c> defaults to
/// <see langword="true"/> in the SDK, so the three tools that only ever <em>add</em> something —
/// opening a pull request, posting a comment, setting one's own review state — say
/// <c>Destructive = false</c> explicitly. Getting that wrong makes a client prompt for confirmation
/// before every comment, or worse, not prompt before a merge.
/// </para>
/// <para>
...
internal sealed class PullRequestWriteTools
{
    ...
    [Description(
        ...
        "braced form ({...}) — read them from getPullRequest on an existing pull request, never guess them " +
        "from names. Calling this twice creates two pull requests, so check with listPullRequests first if " +
    ...
    public static async Task<PullRequestDetail> CreatePullRequestAsync(
        ...
        [Description("Reviewers to request, as Bitbucket account UUIDs in braced form ({...}). Display names, nicknames and emails are rejected.")]
    ...
    public static async Task<PullRequestDetail> UpdatePullRequestAsync(
        ...
        string? destinationBranch = null,
        [Description("The complete new reviewer list, as Bitbucket account UUIDs in braced form ({...}). This REPLACES the existing list; omit to leave the reviewers untouched.")]
        string[]? reviewers = null,
        CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(client);
    ...
    [Description(
        ...
        "actually approved before calling. A strategy the repository has disabled is rejected, not " +
        "substituted, and a conflicting pull request fails with a 409 rather than merging partially.")]
        ...
        foreach (var uuid in cleaned)
        {
            if (!uuid.StartsWith('{') || !uuid.EndsWith('}'))
            {
                throw new McpException(
                    ...
                    "addresses are rejected by Bitbucket. Read the UUIDs from getPullRequest.");
            }
